...
- **Менеджер** оформлює заявку та ТТН, **кладовщик** приймає посилку, розбирає вміст і **закриває** повернення; **менеджер** проводить кредит / повернення коштів за потреби.
...
| **Прийнято на склад** | Нічого — розбирає і **закриває** склад |
| **Перевірка** | За потреби узгодити з клієнтом (якщо ще не закрито складом) |
| **Повернення коштів** | Провести **кредит / повернення грошей** |
| **Закрито** | Готово — закриває **кладовщик** після розбору |

**Важливо:** не проводьте розрахунок без позицій. Якщо було «позиції уточнить склад» — спочатку склад має завершити розбір і закрити.
...
Перевіряйте залишки та артикули перед відвантаженням.`,
  },
  {
    ...
    excerpt: "Черга вхідних посилок: прийом, розбір по замовленнях, закриття.",
    status: "PUBLISHED",
    sortOrder: 25,
    seedRevision: 5,
    visibleRoles: ["WAREHOUSE", "ADMIN", "LEAD"],
...
6. Коли все розкладено → **«Розбір завершено / Закрити»**.

Після цього пов’язані повернення **закриваються**. Розрахунок з клієнтом (кредит / повернення коштів) робить **менеджер**.
...
| Шукаєте замовлення, додаєте позиції | Проводить розрахунок з клієнтом |
| Закриваєте повернення після розбору | — |

**Не робите:** повернення коштів, кредит, створення заявок «з нуля» (лише доповнюєте позиції до вже зареєстрованих посилок).
...
→ У блоці пересорту: **«Очікували»** / **«Фактично»**, оберіть **«На залишок»**, **«Карантин»** або **«Списати»**, збережіть. Після **«Розбір завершено»** повернення закривається (заміна збирається як звичайне **«Підтверджено»** у черзі відвантажень).
