...
export class RiskExposureService {
  ...
  async computeExposure(input: {
    ...
    const orders = await this.prisma.order.findMany({
      where: openDebtWhere,
      select: { debtAmount: true, totalAmount: true, paidAmount: true },
    });

    const utilized = orders.reduce((sum, o) => sum + Number(o.debtAmount ?? 0), 0);
    const additional = input.additionalAmount ?? 0;
    const totalExposure = utilized + additional;
    const creditLimit = profile ? Number(profile.creditLimit) : DEFAULT_CREDIT_POLICY.defaultCreditLimit;
    const availableCredit = Math.max(0, creditLimit - totalExposure);
    const exposurePct = creditLimit > 0 ? Math.round((totalExposure / creditLimit) * 100) : totalExposure > 0 ? 100 : 0;
