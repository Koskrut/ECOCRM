...
type BankProvider = "PRIVAT24" | "UPC";
...
export default function BankSettingsPage() {
  ...
  const availableProviders = useMemo(() => {
    const out: BankProvider[] = [];
    if (effective(ModuleIds.Privat24)) out.push("PRIVAT24");
    if (effective(ModuleIds.Upc)) out.push("UPC");
    return out;
  }, [effective]);
  ...
  const [addProvider, setAddProvider] = useState<BankProvider>("PRIVAT24");
  ...
  const [editProvider, setEditProvider] = useState<BankProvider>("PRIVAT24");
  ...
  }, [availableProviders, addProvider]);
  ...
  function openEdit(acc: BankAccount) {
    setEditId(acc.id);
    setEditProvider(acc.provider ?? "PRIVAT24");
    ...
    try {
      ...
      const res = await (payload
        ? apiHttp.post<RequisitesFromBank>(
            `/integrations/privat24/accounts/${editId}/requisites-from-bank`,
            payload,
          )
  ...
  async function submitEdit() {
    ...
    try {
      ...
      if (
        editProvider === "PRIVAT24" &&
        (editClientIdVal !== "" || editGroupIdVal !== "" || editToken !== "")
      ) {
        body.credentials = {};
        ...
        {showAdd && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-6 shadow-lg">
              <h3 className="text-lg font-semibold text-zinc-900">Додати банківський рахунок</h3>
              <div className="mt-4 space-y-3">
                {availableProviders.length > 1 && (
                  <div>
                    ...
                    <select
                      value={addProvider}
                      onChange={(e) => setAddProvider(e.target.value as BankProvider)}
                ...
                {addProvider === "UPC" && (
                  <p className="text-xs text-zinc-500">
                    Після створення рахунку підключіть банк у{" "}
                    <Link href="/settings/upc" className="text-blue-600 underline">
                      налаштуваннях UPC
                    </Link>
                    .
                  </p>
