...
Текущая линия поставки registry: **`0.2.x`**. Для продакшена рекомендуется актуальный patch (**`0.2.90`**; см. `CHANGELOG.md`). Линия **`0.1.x`** остаётся для уже развёрнутых клиентов до перехода на `0.2.x`.
