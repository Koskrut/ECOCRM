...
Актуализировано под зелёный релиз **`v0.2.129`** (return packages, warehouse returns queue — см. **`CHANGELOG.md` [0.2.129]**).
...
| **`images`** | Есть строки с ролями **`module`** / **`module_outbound`** / **`store`** и тегами **`…:ВАША_ВЕРСИЯ`** (для текущего патча — **`0.2.129`**). |
