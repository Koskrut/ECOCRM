...
Актуализировано под зелёный релиз **`v0.2.67`** (полный манифест CI: **store** + overlays модулей, **`composeFileUrls`**, NP и Google Drive в Settings — см. **`CHANGELOG.md` [0.2.67]**).

## 0. Rollout после зелёного **Publish Registry Release** (CP → сервер)

1. **Control Plane:** для установки выставлен целевой релиз **`0.2.67`**; **`rollouts/next`** (или выдача **`MANIFEST_URL`**) возвращает JSON с **`version`: `0.2.62`**, полным **`composeFiles`** (в т.ч. **`compose.modules.store.yml`**) и **`composeFileUrls`** на каждый путь.
...
| **`images`** | Есть строки с ролями **`module`** / **`module_outbound`** / **`store`** и тегами **`…:ВАША_ВЕРСИЯ`** (для текущего патча — **`0.2.67`**). |
...
- **`BACKEND_VERSION`**, **`WEB_VERSION`**, **`STORE_VERSION`** — совпадают с релизом в registry (целевой патч линии **0.2.x**, сейчас **`0.2.67`**).
