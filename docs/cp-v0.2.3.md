# Control Plane и сервер: манифест релиза (**0.2.93+**), compose, лицензия
...
3. **`gitSha` vs URL в raw:** в raw-URL часто встречается ref вида **`v0.2.93`**, в манифесте — **полный коммит**; это **нормально**, если вы осознанно публикуете оба ref и **содержимое файлов** на GitHub совпадает с релизом.
...
1. **Репозиторий и тег** — согласованы с релизом (например **`v0.2.93`**), есть **`scripts/sync-compose-from-manifest.mjs`** и **`suprex/client-pull-agent.sh`**.
