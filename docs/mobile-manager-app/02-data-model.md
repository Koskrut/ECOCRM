...
Один документ «топливо за день» на пользователя: план км, факт км, `compensationKm` (для выплаты), расчёт литров/суммы, `metricsSource`, `visitCount`, `calculationSnapshot` (JSON визитов), статус компенсации, `submittedAt`, `managerNote`, опционально `shiftId`.
