...
> Синхронизировано с релизом **`v0.2.73`** (полный CI; stock SKU normalizer, entity modals UX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
