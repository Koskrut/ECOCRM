...
> Синхронизировано с релизом **`v0.2.76`** (полный CI; bank Privat24/UPC, CI build fixes): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
