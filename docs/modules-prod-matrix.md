...
> Синхронизировано с релизом **`v0.2.80`** (полный CI; client balances, return settlement, order stage guards): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
