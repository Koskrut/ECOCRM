...
> Синхронизировано с релизом **`v0.2.168`** (shift mobility + debt promises + tasks/leads UX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
