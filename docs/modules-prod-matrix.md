...
> Синхронизировано с релизом **`v0.2.64`** (полный CI: все module sidecars + store; field/mobile API, GPS visits): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
