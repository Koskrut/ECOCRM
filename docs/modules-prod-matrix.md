...
> Синхронизировано с релизом **`v0.2.137`** (MRP sales XLS velocity, action lists, soft reservations): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
