...
> Синхронизировано с релизом **`v0.2.113`** (OSRM match sum-all segments, GPS vs visits fuel sanity, nginx 50M uploads): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
