...
> Синхронизировано с релизом **`v0.2.159`** (task comments/collaborators + packing WIP parts): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
