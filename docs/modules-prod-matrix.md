...
> Синхронизировано с релизом **`v0.2.88`** (FX variance write-off + web CI hotfix): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
