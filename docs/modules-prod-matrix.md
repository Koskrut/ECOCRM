...
> Синхронизировано с релизом **`v0.2.166`** (field shift anchors + visit track contradiction + fuel/route): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
