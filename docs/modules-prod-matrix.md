...
> Синхронизировано с релизом **`v0.2.133`** (employee daily activity dashboard, MRP SKU calc): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
