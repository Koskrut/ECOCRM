...
> Синхронизировано с релизом **`v0.2.111`** (BOM PART auto-create, catalog excludes PART): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
