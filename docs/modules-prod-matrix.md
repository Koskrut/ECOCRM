...
> Синхронизировано с релизом **`v0.2.96`** (Telegram inbox notifications, ad-hoc visit, NP COD, tasks i18n): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
