...
> Синхронизировано с релизом **`v0.2.103`** (manager dashboard inbox/scorecard, Telegram inbound идемпотентность + назначение чата, payments/bank search): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
