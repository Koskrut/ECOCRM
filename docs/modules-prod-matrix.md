...
> Синхронизировано с релизом **`v0.2.107`** (receivables snapshots, Expo push, visit custom location): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
