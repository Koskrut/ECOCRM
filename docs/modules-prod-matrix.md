...
> Синхронизировано с релизом **`v0.2.69`** (полный CI: warehouses CRUD, NP settings pickers, catalog/orders): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
