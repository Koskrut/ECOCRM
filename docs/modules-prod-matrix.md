...
> Синхронизировано с релизом **`v0.2.144`** (shift ops gate, incomplete tour copy): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
