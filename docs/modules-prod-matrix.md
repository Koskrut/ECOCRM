...
> Синхронизировано с релизом **`v0.2.130`** (mis-pick returns, GPS UA filter, mobile session-auth): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
