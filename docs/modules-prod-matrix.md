...
> Синхронизировано с релизом **`v0.2.79`** (полный CI; Kyivstar FMC module, field shifts/team map, mobile GPS tracking): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
