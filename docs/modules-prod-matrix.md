...
> Синхронизировано с релизом **`v0.2.180`** (planning sheet tab + fastener/WIP cover): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
