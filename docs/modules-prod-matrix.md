...
> Синхронизировано с релизом **`v0.2.118`** (Kyivstar FMC fixes, mobile base currency, field shifts BFF): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
