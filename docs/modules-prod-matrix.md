...
> Синхронизировано с релизом **`v0.2.128`** (mobile Android Google Maps native key gate): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
