...
> Синхронизировано с релизом **`v0.2.140`** (MRP false PKG BOM, GPS OSRM payable km): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
