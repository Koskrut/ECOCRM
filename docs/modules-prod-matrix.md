...
> Синхронизировано с релизом **`v0.2.171`** (fuel payout stop-share v2.1 + Kit BOMs list): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
