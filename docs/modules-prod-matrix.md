...
> Синхронизировано с релизом **`v0.2.119`** (bank auto-match v2, FULLY_RETURNED, payer aliases): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
