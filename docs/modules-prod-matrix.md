...
> Синхронизировано с релизом **`v0.2.110`** (OSRM в compose.client.yml для SUPREX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
