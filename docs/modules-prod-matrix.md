...
> Синхронизировано с релизом **`v0.2.102`** (field tracking restart telemetry, bank allocation locks, payment status FX/returns): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
