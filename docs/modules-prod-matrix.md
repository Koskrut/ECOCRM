...
> Синхронизировано с релизом **`v0.2.174`** (Kit BOMs simple/expert UX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
