...
> Синхронизировано с релизом **`v0.2.181`** (inbox UX pin/notes/canned + receivables aging): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
