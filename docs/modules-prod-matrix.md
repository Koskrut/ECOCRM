...
> Синхронизировано с релизом **`v0.2.68`** (полный CI: module sidecars + store; visits plan owner scope для MANAGER): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
