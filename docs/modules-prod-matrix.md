...
> Синхронизировано с релизом **`v0.2.164`** (fuel refuel Kyiv date + warehouse manager + return dialog scroll): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
