...
> Синхронизировано с релизом **`v0.2.67`** (полный CI: module sidecars + store; visits owner scope, voice gateway overlay в репо): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
