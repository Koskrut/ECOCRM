...
> Синхронизировано с релизом **`v0.2.105`** (missed call queue, NP payer/payment mapping, payments infinite scroll): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
