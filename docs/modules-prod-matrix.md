...
> Синхронизировано с релизом **`v0.2.72`** (полный CI; NP TTN edit, employee route addresses, modals): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
