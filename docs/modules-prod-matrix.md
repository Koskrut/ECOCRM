...
> Синхронизировано с релизом **`v0.2.65`** (полный CI: все module sidecars + store; field/fuel, GPS visits, route geometry): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
