...
> Синхронизировано с релизом **`v0.2.178`** (CRM-first demand velocity + planning workplace IA): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
