...
> Синхронизировано с релизом **`v0.2.83`** (hotfix: PrismaService $connect on extended client): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
