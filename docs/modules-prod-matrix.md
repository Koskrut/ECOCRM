...
> Синхронизировано с релизом **`v0.2.129`** (return packages, warehouse returns queue, NP sync): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
