...
> Синхронизировано с релизом **`v0.2.131`** (fuel GPS partial payout, risk pickup ship gate): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
