...
> Синхронизировано с релизом **`v0.2.146`** (material reservations, payment dedup, GPS stale): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
