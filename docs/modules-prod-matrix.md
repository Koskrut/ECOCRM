...
> Синхронизировано с релизом **`v0.2.142`** (shift close reminder, mobile GPS bootstrap): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
