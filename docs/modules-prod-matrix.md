...
> Синхронизировано с релизом **`v0.2.141`** (GPS path integrity, Planning Today): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
