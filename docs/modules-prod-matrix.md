...
> Синхронизировано с релизом **`v0.2.132`** (MRP planning runs, return warehouse binding): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
