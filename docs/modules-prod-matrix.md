...
> Синхронизировано с релизом **`v0.2.108`** (planning pack/factory, OSRM routing, Bitrix legacy debt zero): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
