...
> Синхронизировано с релизом **`v0.2.173`** (fuel payout v2.2 + min pack/produce lots): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
