...
> Синхронизировано с релизом **`v0.2.66`** (полный CI: все module sidecars + store; catalog, visit history, field profile on users): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
