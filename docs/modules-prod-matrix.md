...
> Синхронизировано с релизом **`v0.2.161`** (Product externalCode + pack cycle + modal stack): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
