...
> Синхронизировано с релизом **`v0.2.78`** (полный CI; updater compose service, one-click update UI, warehouse comment/docs): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
