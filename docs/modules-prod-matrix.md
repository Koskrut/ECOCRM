...
> Синхронизировано с релизом **`v0.2.121`** (hotfix backend build; функционально = 0.2.120): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
