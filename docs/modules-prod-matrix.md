...
> Синхронизировано с релизом **`v0.2.101`** (Meta Instagram/Facebook inbox, visit history day map): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
