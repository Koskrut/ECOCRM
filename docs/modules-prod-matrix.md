...
> Синхронизировано с релизом **`v0.2.157`** (1C payments import + tasks urgency): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
