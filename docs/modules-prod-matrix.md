...
> Синхронизировано с релизом **`v0.2.125`** (bank gateway/transit guard, purpose FIO match): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
