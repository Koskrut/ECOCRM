...
> Синхронизировано с релизом **`v0.2.176`** (manager month pulse + growth levers): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
