...
> Синхронизировано с релизом **`v0.2.93`** (shift track geometry, visits Kyiv TZ, leads manager visibility, contacts /code search): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
