...
> Синхронизировано с релизом **`v0.2.99`** (Dashboard v2, visit resultNote edit, mobile reschedule): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
