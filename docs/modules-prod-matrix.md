...
> Синхронизировано с релизом **`v0.2.122`** (bank ignore/technical, Risk credit gates): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
