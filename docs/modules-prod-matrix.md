...
> Синхронизировано с релизом **`v0.2.179`** (GPS telemetry + JWT refresh + shift reopen): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
