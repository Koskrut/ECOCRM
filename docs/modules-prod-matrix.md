...
> Синхронизировано с релизом **`v0.2.134`** (canceled order debt fix, mobile GPS tracking health v2): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
