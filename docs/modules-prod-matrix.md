...
> Синхронизировано с релизом **`v0.2.104`** (fuel refuels с чеками, attention-фильтры tasks/orders/leads, daily agenda summary): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
