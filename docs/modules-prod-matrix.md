...
> Синхронизировано с релизом **`v0.2.147`** (mobile GPS zombie FGS / recovery): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
