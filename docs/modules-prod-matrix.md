...
> Синхронизировано с релизом **`v0.2.100`** (Telegram inbox unread badge, order 1C code UX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
