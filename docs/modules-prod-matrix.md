...
> Синхронизировано с релизом **`v0.2.91`** (daily work agenda, leads convert UX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
