...
> Синхронизировано с релизом **`v0.2.77`** (полный CI; warehouse workspace, stock readiness, updater agent env): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
