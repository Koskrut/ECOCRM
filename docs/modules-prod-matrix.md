...
> Синхронизировано с релизом **`v0.2.75`** (полный CI; bank Privat24/UPC, payment matching; fix google-drive build): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
