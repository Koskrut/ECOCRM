...
> Синхронизировано с релизом **`v0.2.127`** (public Google Maps config, mobile map routing fix): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
