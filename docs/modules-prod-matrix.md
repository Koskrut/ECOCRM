...
> Синхронизировано с релизом **`v0.2.120`** (bank invoice/waybill auto-match, route plan owner guard): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
