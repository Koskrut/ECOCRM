...
> Синхронизировано с релизом **`v0.2.138`** (mobile GPS contour, team GPS tiers): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
