...
> Синхронизировано с релизом **`v0.2.163`** (payments cash split + factory line dueAt + receivables work): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
