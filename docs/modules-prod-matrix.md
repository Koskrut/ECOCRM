...
> Синхронизировано с релизом **`v0.2.70`** (полный CI; NP settings catalog sync UI): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
