...
> Синхронизировано с релизом **`v0.2.150`** (native field tracking + visit GPS ownerId hotfix): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
