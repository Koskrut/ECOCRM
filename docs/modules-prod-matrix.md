...
> Синхронизировано с релизом **`v0.2.124`** (web scroll preserve, payments FX allocate, visits mobile UX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
