...
> Синхронизировано с релизом **`v0.2.87`** (FX variance write-off на заказах USD/EUR): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
