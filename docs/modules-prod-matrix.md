...
> Синхронизировано с релизом **`v0.2.112`** (call recordings на карточках, visit GPS dual-write, OSRM match, fuel track eligibility): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
