...
> Синхронизировано с релизом **`v0.2.149`** (native field tracking, planning due reminders): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
