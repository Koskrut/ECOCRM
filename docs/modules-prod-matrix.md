...
> Синхронизировано с релизом **`v0.2.117`** (Help instruction center, Risk Management ERI, receivables debt comments): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
...
| `ext.risk_management` | extension | core | true | in_process | in-process only (full `AppModule`, not `AppModuleCore`) | — | — | — | — | да | `/risk/*` | `/api/risk/*` | sidebar `/risk` | `RiskCron` nightly 02:00 Kyiv; no-op when module not effective | DB + license | Soft degrade without Finance module. |
