...
> Синхронизировано с релизом **`v0.2.86`** (web: notifications API proxy, entity addresses UX): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
