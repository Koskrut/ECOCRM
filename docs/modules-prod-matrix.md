...
> Синхронизировано с релизом **`v0.2.170`** (product params + ABC–XYZ kit portfolio + CRM-first velocity): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
