...
> Синхронизировано с релизом **`v0.2.74`** (полный CI; WAREHOUSE role, fulfillment queue, route geometry, visits/orders/contacts): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
