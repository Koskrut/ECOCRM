...
> Синхронизировано с релизом **`v0.2.177`** (contacts work-queue UX + reason filters): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
