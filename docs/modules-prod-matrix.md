...
> Синхронизировано с релизом **`v0.2.139`** (company region, required phone/region on create): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
