...
> Синхронизировано с релизом **`v0.2.154`** (route confirm, contact orders movement, awaiting stock): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
