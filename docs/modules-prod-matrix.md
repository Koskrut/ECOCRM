...
> Синхронизировано с релизом **`v0.2.92`** (call-task dedup, warehouses Kyiv/Lutsk/Khmelnytsky, stock upload): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
