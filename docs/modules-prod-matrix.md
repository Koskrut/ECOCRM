...
> Синхронизировано с релизом **`v0.2.123`** (order PDF Cyrillic, GPS keepalive, mobile tracking health): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
