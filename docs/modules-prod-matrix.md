...
> Синхронизировано с релизом **`v0.2.71`** (полный CI; contacts modal UX, region required): NP **IntegrationSetting** + UI **`/settings/nova-poshta`**; Google Drive в **Settings → Google-таблиця**; companies create (name-only, auto owner); rewrite путей прокси NP → upstream.
