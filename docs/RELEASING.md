...
Актуальная линия registry: **`0.2.x`**. Рекомендуемый стабильный patch для продакшена: **`v0.2.70`** / **`0.2.70`** (образы после успешного **Publish Registry Release**). Более старые патчи **`0.2.0`** … **`0.2.69`** для **`crm-backend-core`** на бою не рекомендуются без причины — см. `CHANGELOG.md` **[0.2.70]** и предыдущие секции. Предыдущая линия **`0.1.x`** — для клиентов до перехода.
...
git tag -a v0.2.70 -m "Release 0.2.70"
git push origin v0.2.70
```

После `git push origin v0.2.70` запустится workflow **Publish Registry Release** (триггер `push` тегов `v*`). Для **`v0.2.*`** при пустом **modules** в dispatch собираются **все** module-образы и полный манифест compose.
...
**Actions → Publish Registry Release → Run workflow** — поле `version`: `0.2.70`, при необходимости поле **modules** (пусто для `0.2.x` = все модули), ветка с нужным коммитом.

## После успешного CI

- Обновить у клиентов **`BACKEND_VERSION` / `WEB_VERSION` / `STORE_VERSION`** в `.env` на **`0.2.70`**, `docker compose pull` и перезапуск.
