...
Актуальная линия registry: **`0.2.x`**. Рекомендуемый стабильный patch для продакшена: **`v0.2.4`** / **`0.2.4`** (образы после успешного **Publish Registry Release**). Теги **`0.2.0`** / при необходимости ранние **`0.2.1`** / **`0.2.2`** / **`0.2.3`** для **`crm-backend-core`** на бою не используйте без необходимости — см. `CHANGELOG.md` **[0.2.4]**, **[0.2.3]**, **[0.2.2]**. Предыдущая линия **`0.1.x`** — для клиентов до перехода.
...
После `git push origin v0.2.4` запустится workflow **Publish Registry Release** (триггер `push` тегов `v*`).
...
**Actions → Publish Registry Release → Run workflow** — поле `version`: `0.2.4`, при необходимости поле **modules** (пусто для `0.2.x` = все модули), ветка с нужным коммитом.

## После успешного CI

- Обновить у клиентов **`BACKEND_VERSION` / `WEB_VERSION` / `STORE_VERSION`** в `.env` на **`0.2.4`**, `docker compose pull` и перезапуск.
