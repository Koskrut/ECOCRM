...
- Control Plane должен **сохранять и отдавать** поле **`composeFileUrls`** при регистрации релиза и в ответах вроде **`rollouts/next`** (как в JSON из CI). Хотфикк compose в CP: **`PATCH …/deployment-manifests/:id/compose`** с **`policy: "passthrough"`** или **`"ci_urls"`** — см. **`docs/cp-v0.2.3.md`** и документы в репозитории CP (`docs/releasing-manifest.md`, `docs/deployment-manifest-compose-patch.md`). Если CP обрезает поля — обновите схему/миграции или временно кладите актуальный `deployment-manifest.json` в корень bundle.
