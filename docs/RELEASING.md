...
Актуальная линия registry: **`0.2.x`**. Последний подготовленный в репозитории минор: **`v0.2.0`** / **`0.2.0`** (образы после успешного **Publish Registry Release**). Предыдущая линия **`0.1.x`** — для клиентов до перехода.
...
После `git push origin v0.2.0` запустится workflow **Publish Registry Release** (триггер `push` тегов `v*`).
...
**Actions → Publish Registry Release → Run workflow** — поле `version`: `0.2.0`, при необходимости поле **modules** (пусто для `0.2.x` = все модули), ветка с нужным коммитом.

## После успешного CI

- Обновить у клиентов **`BACKEND_VERSION` / `WEB_VERSION` / `STORE_VERSION`** в `.env` на **`0.2.0`**, `docker compose pull` и перезапуск.
